...
class JLeagueRankingParserTest {
    ...
    fun whenParsingTestDataFirstIsKashima() {
        ...
        assertThat(list[2].name, `is`("ガンバ大阪"))
        assertThat(list[3].name, `is`("ＦＣ東京"))
        ...
        assertThat(list[10].name, `is`("横浜Ｆ・マリノス"))
        assertThat(list[11].name, `is`("ヴァンフォーレ甲府"))
        assertThat(list[12].name, `is`("ベガルタ仙台"))
        assertThat(list[13].name, `is`("清水エスパルス"))
        assertThat(list[14].name, `is`("北海道コンサドーレ札幌"))
        assertThat(list[15].name, `is`("サンフレッチェ広島"))
        assertThat(list[16].name, `is`("アルビレックス新潟"))
        assertThat(list[16].name, `is`("大宮アルディージャ"))
    }
